...
class QAgent_Maximax:
    ...
    def maximax_future(self, next_state, next_available, opponent):
        # Compute maximax assuming opponent plays to maximize payoff
    ...
    def update(self, state, action, reward, next_state, next_available, opponent):
        ...
        if reward != 0 or not next_available:
            ...
            future = self.maximax_future(next_state, next_available, opponent)
...
def evaluate(agent_X, agent_O, games=5000, alternate_starts=True):
...
if __name__ == "__main__":
    ...
    results = evaluate(agent_X, agent_O, games=5000, alternate_starts=True)
    print("Evaluation results:", results)

    play_match(agent_X, agent_O)

    # Save Q-tables
    with open("qtables/agent_X_qtable_maximax.pkl", "wb") as f:
        pickle.dump(dict(agent_X.q_table), f)

    with open("qtables/agent_O_qtable_maximax.pkl", "wb") as f:
